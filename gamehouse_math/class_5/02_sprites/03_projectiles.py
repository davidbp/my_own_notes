...
class player(object):
    ...
    def draw(self, window):
        ...
        if not(self.standing):
            if self.left:
                window.blit(walkLeft[self.walkCount//SPRITE_DIV], (self.x,self.y))
                self.walkCount += 1
            elif self.right:
                window.blit(walkRight[self.walkCount//SPRITE_DIV], (self.x,self.y))
                self.walkCount +=1
        else:
            if self.right:
...
while run:
    ...
    for bullet in bullets:
        if bullet.x < WINDOW_X and bullet.x > 0:
            bullet.x += bullet.vel
        else:
    ...
    if keys[pygame.K_LEFT] and man.x > man.vel:
        ...
        man.right = False
        man.standing = False
    elif keys[pygame.K_RIGHT] and man.x < WINDOW_X - man.width - man.vel:
        man.x += man.vel
        man.right = True
        man.left = False
        man.standing = False
