...
//
 void 
    DFS_Trees( graph &G, int &DFSn, vector<size_t> &DFSind, vector<vertex> &DFSp, vector<length> &DFSd, vertex v ) 
{
    vertex vn = G.size();
    for( size_t i=0; i<G[v].size(); i++ ) {
         if( DFSp[G[v][i]]==vn ) {
...
//  Find non parented vertex as root vertex rv of new component/tree Tn
	for( vertex rv=0; rv<vn; rv++ ) 
        if( DFSp[rv]==vn ) {
			Tn++;
		//  size_t, parent and depth for rv and recursive DFS from rv
			DFSind[rv] = DFSn++;
            DFSp[rv] = rv;    		
			DFSd[rv] = 0;
...
//
 void
    Dijkstra2( graph &G, size_t n1, size_t n2, size_t s1, size_t s2,  ofstream& fout ) 
{
    vertex vn = G.size();
    vertex sv = s1*n2+s2;
//  Inialize vertices as unlabeled
